...
リポジトリは parse-result JSON の契約を検証できる JSON Schema を `schema/` 配下に提供しなければならない (MUST)。この Schema は `ok` と `parse_error` の parse-result JSON のみを対象とし、text 出力経路を対象に含めてはならない (MUST)。
