...
Gemfile.lock は通常 `bundle install` で自動生成されるため、入力の揺れを広く許容するよりも、典型的な出力に対して厳密に解釈し、依存グラフを正しく構築できない場合は fatal とする方針を採る。一方で、`PLATFORMS`、`RUBY VERSION`、`BUNDLED WITH` は必須情報ではないため、欠損や不完全さは warning として扱う。
...
- V1 の対象外である `GIT` / `PATH` / `PLUGIN` を明示的に扱い、誤って通常解決対象として混ぜない。
...
採用案:
- `LineKind` で `Blank`、`TopLevelHeader(String)`、`SpecsHeader`、`IndentedEntry { indent, text }`、`SpecEntry(String)`、`SpecDependencyEntry(String)`、`Unknown(String)` を分類する。
- `ParserState` は `current_section`、`current_spec_name`、`line`、`seen_sections`、`ignored_resolved_names` と構築中の結果を持つ。
...
`GIT` / `PATH` / `PLUGIN` は V1 の解析対象外とし、未対応セクションとして基本は `IgnoredSection` warning を返す。`DEPENDENCIES` のエントリ末尾に `!` がある場合は、その依存が通常解決対象ではない明示的なシグナルとして `UnsupportedResolvedSource` の fatal とする。
...
- optional セクションの見出しは存在するが本文が不足または不正な場合は `IncompleteOptionalSection` warning を返し、fatal にはしない。
...
- [未対応 source 判定が限定的] → `UnsupportedResolvedSource` は `DEPENDENCIES` の `!` に限定し、それ以外の未対応セクションは `IgnoredSection` として扱う制限を仕様に明記する。
